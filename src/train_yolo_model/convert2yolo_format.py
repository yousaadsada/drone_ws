...
def split_csv_and_convert_to_yolo(csv_file, img_dir, output_dir, train_ratio=0.8):
    ...
    def process_subset(subset_df, img_output_dir, label_output_dir):
        for _, row in subset_df.iterrows():
            ...
            img_path = os.path.join(img_dir, f"jackal_raw_{image_id}.png")
...
if __name__ == "__main__":
    # Paths
    csv_file = os.path.join(os.path.expanduser("~"), 'drone_ws', 'data', 'keypoint','keypoints.csv')
    img_dir = os.path.join(os.path.expanduser("~"), 'drone_ws', 'data', 'jackal_figure','raw')
    output_dir = os.path.join(os.path.expanduser("~"), 'drone_ws', 'src', 'train_yolo_model', 'dataset')
