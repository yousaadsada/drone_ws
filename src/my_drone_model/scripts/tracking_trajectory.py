...
class DroneTrajectory:
    def __init__(self):
        ...
        # self.data_collection_flag_publisher = rospy.Publisher('/data_collection_flag', Bool, queue_size=1)
        ...
        self.user_input_r = 0.0
        self.user_input_h = 0.0

        self.xy_period_time = 20.0
        self.z_period_time = 20.0
        self.time_stamp = 0.0

        self.data_collection_flag = False
        self.collecting_process = False
        self.initialize = 'process'
        ...
        self.data_dir = os.path.join(os.path.expanduser("~"), 'drone_ws', 'data', 'tracking_trajectory')
        os.makedirs(self.data_dir, exist_ok=True) 

        self.trajectory_update_thread = threading.Thread(target=self.trajectory_update_thread_callback)
        self.trajectory_update_thread.daemon = True
        self.trajectory_update_thread.start()
    ...
    def run_gui(self):
        self.root = tk.Tk()
        self.root.title("Radius and Height Input")

        self.entries = []
        self.labels = ["radius", "height"]
    ...
    def send_data(self):
        try:
            self.user_input_r = float(self.entries[0].get())
            self.user_input_h = float(self.entries[1].get())
            self.collecting_process = True
            self.time_stamp = 0.0

            self.csv_file = os.path.join(self.data_dir, f'state_data_{self.user_input_r}_{self.user_input_h}.csv')
            self.write_csv_header()  # Write CSV header after setting the file name

        except ValueError:
            rospy.logwarn("Invalid input detected.")
    ...
    def write_csv_header(self):
        with open(self.csv_file, mode='w', newline='') as file:
            ...
            writer.writerow(['elapsed_time',
                             ...
                             'ref_x_speed', 'ref_y_speed', 'ref_z_speed', 'ref_yaw_speed',
                             'current_x', 'current_y', 'current_z', 'current_yaw',
                             'current_x_speed', 'current_y_speed', 'current_z_speed', 'current_yaw_speed',
    ...
    def collecting_data_thread_callback(self):
        while self.running:
            if self.data_collection_flag == True:
    ...
    def trajectory_update_thread_callback(self):
        while self.running:
            start_time = rospy.get_time()
            if self.collecting_process == True:
                if self.initialize == "process":
                    self.ref_x = self.user_input_r + 2.0
                    self.ref_y = 0.0
                    self.ref_z = self.user_input_h
                    self.ref_yaw = math.pi
                    self.ref_x_speed = 0.0
                    self.ref_y_speed = 0.0
                    self.ref_z_speed = 0.0
                    self.ref_yaw_speed = 0.0
                    ...
                    if abs(self.current_drone_state.x - self.ref_x) < 0.05 and \
                       abs(self.current_drone_state.y - self.ref_y) < 0.05 and \
                       abs(self.current_drone_state.z - self.ref_z) < 0.05 and \
                       abs(self.current_drone_state.yaw - self.ref_yaw) < 0.05:
                    
                       self.initialize = "complete"
                    else:
                        pass

                elif self.initialize == "complete":
                    self.ref_x = self.user_input_r * math.cos(2 * math.pi * self.time_stamp / self.xy_period_time) + 2.0
                    self.ref_y = self.user_input_r * math.sin(2 * math.pi * self.time_stamp / self.xy_period_time)
                    self.ref_z = self.user_input_h + 0.5 * math.sin(2 * math.pi * self.time_stamp / self.z_period_time)
                    self.ref_yaw = math.atan2(self.ref_y, self.ref_x - 2.0) + math.pi
                    
                    w_xy = 2 * math.pi / self.xy_period_time
                    w_z  = 2 * math.pi / self.z_period_time

                    dx   = -self.user_input_r * w_xy * math.sin(w_xy * self.time_stamp)
                    dy   =  self.user_input_r * w_xy * math.cos(w_xy * self.time_stamp)
                    dz   =  0.5 * w_z * math.cos(w_z * self.time_stamp)

                    # yaw rate using atan2 derivative
                    num = (self.ref_x - 2.0) * dy - self.ref_y * dx
                    den = (self.ref_x - 2.0)**2 + self.ref_y**2
                    dyaw = num / den

                    self.ref_x_speed = dx
                    self.ref_y_speed = dy
                    self.ref_z_speed = dz
                    self.ref_yaw_speed = dyaw
