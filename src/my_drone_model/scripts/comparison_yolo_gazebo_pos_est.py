...
class ManualRollControl:
    def __init__(self):
        ...
        self.jackal_state_yolo_raw = DroneState()
        ...
        self.data_dir = os.path.join(os.path.expanduser("~"), 'drone_ws', 'data', 'yolo_gazebo_pos_est')
        ...
        self.subscribe_jackal_pos_yolo = rospy.Subscriber('/jackal_pos_yolo', DroneState, self.subcribe_jackal_pos_yolo_raw_callback, queue_size=1)
    ...
    def run_gui(self):
        ...
        self.start_button = tk.Button(self.root, text="Start Collecting", command=self.start_collecting)
    ...
    def start_collecting(self):
        ...
        self.time_stamp = 0.0
        self.csv_file = os.path.join(self.data_dir, f'yolo_gazebo_pos_est.csv')
        ...
        while self.running:
            if self.is_collecting_on == True:
                ...
                data = [self.time_stamp,
                        ...
                        self.jackal_state_yolo_raw.x,
                        self.jackal_state_yolo_raw.y,
                        self.jackal_state_yolo_raw.z,
                        self.jackal_state_yolo_raw.x_speed,
                        self.jackal_state_yolo_raw.y_speed,
                        self.jackal_state_yolo_raw.z_speed,
                        ]
    ...
    def subcribe_jackal_pos_yolo_raw_callback(self, msg):
        if msg.target == True:
            self.jackal_state_yolo_raw.x = msg.x
            self.jackal_state_yolo_raw.y = msg.y
            self.jackal_state_yolo_raw.z = msg.z
            self.jackal_state_yolo_raw.x_speed = msg.x_speed
            self.jackal_state_yolo_raw.y_speed = msg.y_speed
            self.jackal_state_yolo_raw.z_speed = msg.z_speed
    ...
    def subscribe_jackal_pos_vel_kalman_filter_callback(self, msg):
        ...
        if self.is_collecting_on == False:
            self.ref_camera_pitch = 0.8
        
        elif self.is_collecting_on == True:
            ...
            panel_distance = max(math.sqrt(dx**2 + dy**2), 1e-6)
    ...
    def pub_jackal_command_thread_callback(self):
        while self.running:
            ...
            if self.is_collecting_on == True:
                self.jackal_command.linear.x = 1.0
                self.jackal_command.angular.z = 1.0
                self.jackal_command_publisher.publish(self.jackal_command)
                self.time_stamp += 0.01

                if self.time_stamp >= 6.28:
                    self.is_collecting_on = False
